...
func (t *Tools) Setup() {
	...
	if t.Clean.Status {
		t.Clean.name = "Clean"
		t.Clean.isTool = true
		t.Clean.cmd = []string{gocmd, "clean"}
	...
	if t.Generate.Status {
		...
		t.Generate.cmd = []string{gocmd, "generate"}
...
func (t *Tool) Compile(path string, stop <-chan bool) (response Response) {
	...
	ppp, _ := filepath.Abs(t.Dir)

	args = append(args, ppp)
